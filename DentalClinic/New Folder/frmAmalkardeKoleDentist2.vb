Public Class frmAmalkardeKoleDentist
    ...
    Private Sub btnGenerate_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnGenerate.Click
        ...
        '----------------------- Insert Into DentistHesabdari ---------------------------------------
        Dim cmd As New SqlClient.SqlCommand
        cmd.CommandText = "insert into dentisthesabdari (DentistID,DentistName,Date,Day,Pardakhti,Sum) Values ('" & DentistID & "','" & Me.cboDentistName.Text & "','" & Tarikh & "','" & Day & "','" & Me.txtPardakhti.Text & "','" & Me.lblSumIndore.Text & "')"
        ...
        cmdUpdate.CommandText = "update sum_amalkardeKoleDentist set dentistname='" & Me.cboDentistName.Text & "',date1='" & Date1 & "',date2='" & Date2 & "',SumIndore='" & Me.lblSumIndore.Text & "',sumKol='" & Me.lblSumKol.Text & "',mandeazghabl='" & Me.lblMandeAzGhabl.Text & "',pardakhti='" & Me.txtPardakhti.Text & "',baghimande='" & Me.txtBaghimande.Text & "'"
    ...
    Private Sub SUM_Amalkard()
        ...
        Dim cmd As New SqlClient.SqlCommand("SELECT sum((daryafti * '" & Percent & "')/100)+sum(kosoorat)'SahmeNahaee' FROM hesabdari where DentistName='" & Me.cboDentistName.Text & "' and Date between '" & Me.txtDate1.Text & "' and '" & Me.txtDate2.Text & "'", SQL.sqlcon)
        ...
        If sdr.Read Then
            If Not sdr(0).ToString = String.Empty Then
                Me.lblSumIndore.Text = sdr(0).ToString
                Done = True
            Else
                SQL.sqlcon.Close()
    ...
    Public Sub GetDentistTalab()
        Dim cmd As New SqlClient.SqlCommand("SELECT sum(Pardakhti-Sum) from DentistHesabdari group by dentistID Having date< N'" & Me.txtDate2.Text & "' and DentistID='" & DentistID & "'", SQL.sqlcon)
